...
void attitudeCalculation(void)
{
    ...
    tmp = (int16_t) rawSensorData.position * 66;
    attitude.position = tmp;

    tmp = (int16_t) rawSensorData.diffSide * 133;
    attitude.diffSide = tmp;

    attitude.angleAcc = (int32_t)-100 * atan2_cordic(attitude.longAcc, attitude.vertAcc);
